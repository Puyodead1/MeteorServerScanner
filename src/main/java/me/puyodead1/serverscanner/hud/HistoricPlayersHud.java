package me.puyodead1.serverscanner.hud;
...
public class HistoricPlayersHud extends HudElement {
    public List<ServersResponse.PlayerSample> players = List.of();
    ...
    private final Setting<Boolean> showCrackedText = sgGeneral.add(new BoolSetting.Builder().name("show-cracked-text").description("Shows the cracked text.").defaultValue(true).build());

    private final Setting<Boolean> showCurrentPlayers = sgGeneral.add(new BoolSetting.Builder().name("show-current-players").description("Shows players that are currently on the server.").defaultValue(true).build());

    private final Setting<Boolean> showWhenEmpty = sgGeneral.add(new BoolSetting.Builder().name("show-when-empty").description("Shows the hud even when there are no historic players.").defaultValue(true).visible(() -> !showCurrentPlayers.get()).build());

    private final Setting<SettingColor> currentPlayersColor = sgGeneral.add(new ColorSetting.Builder().name("current-players-color").description("The color of the current players text.").defaultValue(new SettingColor(255, 255, 255)).visible(showCurrentPlayers::get).build());

    private final Setting<SettingColor> historicPlayersColor = sgGeneral.add(new ColorSetting.Builder().name("historic-players-name-color").description("The color of the historic players text.").defaultValue(new SettingColor(255, 255, 255)).build());

    private final Setting<SettingColor> historicPlayersLastSeenColor = sgGeneral.add(new ColorSetting.Builder().name("last-seen-color").description("The color of the historic players last seen text.").defaultValue(new SettingColor(175, 175, 175)).build());

    private final Setting<Integer> limit = sgGeneral.add(new IntSetting.Builder().name("limit").description("The maximum amount of players to display.").defaultValue(10).min(1).max(1000).noSlider().build());

    private final Setting<Alignment> alignment = sgGeneral.add(new EnumSetting.Builder<Alignment>().name("alignment").description("Horizontal alignment.").defaultValue(Alignment.Auto).build());
    ...
    public void render(HudRenderer renderer) {
        ...
        List<ServersResponse.PlayerSample> players = new ArrayList<>(this.players);
