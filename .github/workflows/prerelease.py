...
def main():
    ...
    if GITHUB_TOKEN is not None:
        ...
        get_tags = requests.get(
            f"https://api.github.com/repos/Puyodead1/MeteorServerScanner/releases/tags/latest",
            headers={
                "Authorization": f"Bearer {GITHUB_TOKEN}"
            },
        )
        if get_tags.status_code == 200:
            ...
            del_req = requests.delete(
                f"https://api.github.com/repos/Puyodead1/MeteorServerScanner/releases/{release_id}",
                headers={
                    "Authorization": f"Bearer {GITHUB_TOKEN}"
                }
            )
